...
Access to the feature is limited by a credit system, where each test generated uses 1 credit. Usage is metered based on the number of tests generated per calendar month, and reset on the 1st day of each month.

Credits are alotted to your account based on your plan:
...
Note that credits are allocated to the organisation, not individual users.
...
:::note
The limits and your usage against these limits are not currently visible on the subscription page. They may be checked via the `pactflow-ai status` command.
:::
...
To enable AI access for users, ensure that the assigned role includes either the [`ai:*`](/docs/permissions#ai) permission (granting access to all AI features) or the specific permissions required for the desired functionality. Once granted, access may be checked via the `pactflow-ai status` command.
