...
|Inheritance and Polymorphism|Y|Inheritance is supported via the use of the `discriminator` keyword. For inheritance to work, the value of `discrimator` should be narrowly specified by `enum` to a set that restricts its value to the desired possible schema subtypes. Otherwise, the subtype schema will not be restricted, and any input will pass. Where the polymorphic content is supplied (at the level of the keyword), you need to set the `additionalProperties: true`.|[Example](https://github.com/pactflow/bdct-oas-examples/tree/main/examples/inheritance)|
