  ...
  def absolutize_links(contents, source_repository_slug, path_transformer, contents_source_path, source_paths)
    contents.gsub(/\]\(([^)]+)\)/) { | match |
      ...
      if url.start_with?('http', '#')
        ...
        parts = url.split(" ")
        absolute_url = (Addressable::URI.parse("https://raw.githubusercontent.com/#{source_repository_slug}/master/#{parts[0]}")).to_s
        "](#{absolute_url})"
      else
        ...
        # absolute_url = (Addressable::URI.parse("https://raw.githubusercontent.com/#{source_repository_slug}/master/#{parts[0]}")).to_s
        absolute_url = (Addressable::URI.parse("https://raw.githubusercontent.com/#{source_repository_slug}/master/") + path_from_root).to_s
        "](#{absolute_url})"
      end
    }
  end
