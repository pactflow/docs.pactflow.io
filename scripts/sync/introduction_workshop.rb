...
SOURCE_REPO = 'pact-foundation/pact-workshop-js'
DESTINATION_DIR = relative_path_to('website/docs/docs/workshops/introduction/')
TRANSFORM_PATH = -> (path) { File.join(DESTINATION_DIR, path.downcase) }
INCLUDE = [
  ->(path) { %w{README.md LEARNING.md}.include?(path) }
]
IGNORE = []
